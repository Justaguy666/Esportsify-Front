import { player_registration_forms } from '../data/sample.js';

class RegistrationPlayerManagement {
    constructor() {
        this.registrationForms = player_registration_forms;
        this.currentForm = null;
        console.log('RegistrationPlayerManagement initialized');
        console.log('Player registration forms:', this.registrationForms);
        this.init();
    }

    init() {
        console.log('Initializing registration player management');
        ...
        formDiv.innerHTML = `
            <div class="form-header-container">
                <div class="form-header-left">
                    <div class="form-header-name"><h1>${form.player_name}</h1></div>
                    <div class="form-header-tag-name">${form.handle}</div>
            ...
            <div class="form-body-container">
                ...
                <div class="manager-details">
                    <div class="manager-info-container">
                        <div class="manager-info"><span>Email:</span> ${form.email}</div>
                        <div class="manager-info"><span>Submitted:</span> ${form.submitted}</div>
                    </div>
                    <div class="manager-info-container">
                        <div class="manager-info"><span>Phone:</span> ${form.phone}</div>
                        <div class="manager-info"><span>Handle:</span> ${form.handle}</div>
                    </div>
                </div>
            </div>
        `;
    ...
    searchForms(query) {
        ...
        return this.registrationForms.filter(form => 
            form.player_name.toLowerCase().includes(lowercaseQuery) ||
            form.handle.toLowerCase().includes(lowercaseQuery) ||
            form.game.toLowerCase().includes(lowercaseQuery) ||
            form.tournament.toLowerCase().includes(lowercaseQuery) ||
            form.email.toLowerCase().includes(lowercaseQuery)
        );
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    new RegistrationPlayerManagement();
});

export default RegistrationPlayerManagement;

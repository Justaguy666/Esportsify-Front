...
export function renderPlayerTableRows(playersPageItems) {
    const tbody = document.getElementsByTagName('tbody')[0];
    tbody.classList.remove('animate');
    tbody.innerHTML = '';
    
    // Render player rows
    playersPageItems.forEach(player => {
        const row = document.createElement('tr');
        row.innerHTML = `
            <td>${player.id}</td>
            <td>
                <div class="avatar">
                    <div class="avatar-circle">${getInitials(player.name)}</div>
                    <span>${player.name}</span>
                </div>
            </td>
            <td><span class="team-tag">${player.teamTag}</span></td>
            <td>${player.game}</td>
            <td>${player.team}</td>
            <td>
                <div class="actions">
...
// Export function to render team table  
export function renderTeamTableRows(teamsPageItems) 
{
    const tbody = document.getElementsByTagName('tbody')[0];
    tbody.classList.remove('animate');
    tbody.innerHTML = '';
    ...
    // Render team rows
    teamsPageItems.forEach(team => {
        const row = document.createElement('tr');
        row.innerHTML = `
            <td>${team.id}</td>
            <td>
                <div class="avatar">
                    <div class="avatar-circle">${getInitials(team.name)}</div>
                    <span>${team.name}</span>
                </div>
            </td>
            <td><span class="team-tag">${team.teamTag}</span></td>
            <td>${team.game}</td>
            <td>
                <div class="actions">   
                    <button class="edit-btn" title="Edit">
